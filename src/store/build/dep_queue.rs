...
pub struct DependencyQueue<N: Hash + Eq, E: Hash + Eq, V> {
  ...
  pub(crate) dep_map: HashMap<N, (HashSet<(N, E)>, V)>,
  ...
  /// The relative priority of this package. Higher values should be scheduled
  /// sooner.
...
impl<N: Hash + Eq + Clone, E: Eq + Hash + Clone, V> DependencyQueue<N, E, V> {
  ...
  /// this node. This implementation does not care about the units of this
  /// value, so the calling code is free to use whatever they'd like. In
  /// general, higher cost nodes are expected to take longer to build.
  pub fn queue(
    &mut self,
    key: N,
    value: V,
  ...
  pub fn queue_finished(&mut self) {
    ...
    /// set of nodes which depend on it, including transitively. This is
    /// different from self.reverse_dep_map because self.reverse_dep_map
    /// only maps one level of reverse dependencies.
